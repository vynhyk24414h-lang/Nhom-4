...
# =========================================================
  ...
  # Số vốn tối đa phân bổ cho mỗi vị thế
  ...
  # Nếu ngày không hợp lệ thì chuyển thành NaT
  ...
  # Chuyển Open và Close về dạng số
  # Nếu dữ liệu không thể chuyển thành số thì gán NaN
  ...
  for ticker, group in data.groupby("ticker"):
      ...
      for i in range(len(group) - 1):
          ...
          # Chỉ tạo sự kiện nếu có tín hiệu BUY hoặc SELL
  ...
  # Ưu tiên xử lý SELL trước BUY nếu cùng ngày
  events = sorted(
      events,
      key=lambda x: (
          x["execution_date"],
          0 if x["signal"] == "SELL" else 1
      )
  )
  ...
  for event in events:
      ...
      # =========================================================
          ...
          if len(positions) < max_positions:
              ...
              # Số vốn tối đa dành cho giao dịch này
              ...
              # Tính số cổ phiếu tối đa có thể mua
              raw_shares = (
                  available_budget
                  / (price * (1 + transaction_fee_rate))
              )


              # Làm tròn xuống theo lô giao dịch
              shares = (
